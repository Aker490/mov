...
if(isset($_POST['username_account']) && isset($_POST['password_account'])){
    ...
    if(mysqli_num_rows($call_back_check_account) == 1){
        ...
        if($result_check_account['lock_account'] == 1){
            ...
            echo '<a href="index-login.php">กลับไปยังหน้าเข้าสู่ระบบ</a>';
            ...
            if($result_check_account['role_account'] == 'member'){ //บทบาท member
                ...
                $_SESSION['id_account'] = $result_check_account['id_account'];
                $_SESSION['role_account'] = $result_check_account['role_account'];
                die(header('Location: ../Dashboard/Dashboard.php'));
            }
        }else{
            $query_login_count_account = "UPDATE account SET login_count_account = login_count_account + 1 WHERE username_account = '$username_account'";
            ...
            if($result_check_account['login_count_account'] + 1 >= $limit_login_account){
                ...
                $query_lock_account = "UPDATE account SET lock_account = 1, ban_account = DATE_ADD(NOW(), INTERVAL $time_ban_account MINUTE) WHERE username_account = '$username_account'";
                $call_back_lock_account = mysqli_query($connect, $query_lock_account);
            }
            die(header('Location: index-login.php')); //รหัสผ่านไม่ถูกต้อง
        }

    }else{
        die(header('Location: index-login.php')); //ไม่มีอีเมลนี้ในระบบ
    }

}else{
    die(header('Location: index-login.php')); //กรุณากรอกข้อมูล
}

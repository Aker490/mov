...
if (!$list) {
    // ดึงข้อมูลภาพยนตร์
    $stmt = $con->prepare("SELECT * FROM data_movie_thai WHERE id = ?");
    $stmt->bind_param("i", $id);
} else {
    // ดึงข้อมูลตอน
    $stmt = $con->prepare("SELECT * FROM data_list_thai WHERE main_id = ? AND episode = ?");
...
$num_list = mysqli_num_rows(mysqli_query($con, "SELECT * FROM data_list_thai WHERE main_id = $id"));
?>
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="play01">
                ...
                <div class="col-12 text-center">
                    <div class="list-group list-group-horizontal-sm" id="list-tab" role="tablist">
                        <a class="list-group-item list-group-item-action active" data-bs-toggle="list"
                            href="#play01">Play 1</a>
                        <a class="list-group-item list-group-item-action" data-bs-toggle="list" href="#play02">Play
                            2</a>
                        <a class="list-group-item list-group-item-action" data-bs-toggle="list" href="#play03">Play
                            3</a>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-4">
                        <a class="btn mb-1 shadow-sm text-center <?php if($list <= 1){echo "disabled";}?> "
                            style="background-color: rgb(41, 36, 36);" href="play.php?id=<?=$id?>&list=<?=$list-1?>">
                    ...
                    <div class="col-md-4">
                        <a class="btn mb-1 shadow-sm text-center" style="background-color: rgb(41, 36, 36);"
                            href="./list.php?id=<?=$id?>">
                            <h2 style="color: rgb(255, 255, 255);margin: 0;padding: 10px;width: 100%;">
                                ตอนอื่นๆ
                            </h2>
                    ...
                    <div class="col-md-4">
                        <a class="btn mb-1 shadow-sm text-center <?php if($list >= $num_list){echo "disabled";}?> "
                            style="background-color: rgb(41, 36, 36);" href="play.php?id=<?=$id?>&list=<?=$list+1?>">
                            <h2 style="color: rgb(255, 255, 255);margin: 0;padding: 10px;width: 100%;">
                                ตอนถัดไป
                            </h2>
